...
class MelSpectrogram(torch.nn.Module):
    ...
    def forward(self, audio, keyshift=0, speed=1, center=True):
        ...
        if audio.device.type in ("xpu", "privateuseone"):
            if (
                not hasattr(self, "stft")
                or self.stft.filter_length != n_fft_new
                or self.stft.hop_length != hop_length_new
                or self.stft.win_length != win_length_new
            ):
                self.stft = STFT(
